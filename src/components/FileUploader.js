...
const FileUploader = ({
  onDataLoaded,
  onTTSLanguageChange,
  onSelectedLanguageChange, // Новый пропс
  data,
  firstElement,
  setFirstElement,
  ttsLanguage,
  selectedLanguage, // Новый пропс
  languages,
}) => {
  ...
  const analyzeFileName = (fileName) => {
    ...
    if (
      ...
      // Преобразуем sourceLang в формат ttsLanguage (например, 'de' -> 'de-DE')
  ...
  const handleSelectFile = (event) => {
    ...
    if (selectedFileName) {
      analyzeFileName(selectedFileName); // Анализируем имя файла
